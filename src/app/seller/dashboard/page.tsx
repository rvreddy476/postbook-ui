...
export default function SellerDashboardPage() {
  ...
  if (!seller) {
    return (
      <AppShell activeTab="Shop">
        <div className="min-h-screen flex items-center justify-center bg-secondary">
          <div className="bg-white rounded-2xl shadow-lg border border-border p-10 max-w-md text-center">
            ...
            <h2 className="text-2xl font-black text-foreground mb-2">Start Selling on VChat</h2>
            <p className="text-muted-foreground mb-6 text-sm">Reach millions of users. Set up your shop in minutes.</p>
            <button
              onClick={() => router.push('/seller/onboarding')}
              className="w-full py-3.5 bg-[#1A1A1A] text-white rounded-xl font-bold hover:bg-foreground transition text-sm"
            >
              Open a Shop
            </button>
          </div>
    ...
    <AppShell activeTab="Shop">
      <div className="min-h-screen bg-secondary">
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          {seller.status === 'draft' && (
            <div className="mb-6 bg-white border border-border rounded-2xl p-5 flex items-center justify-between">
              <p className="text-muted-foreground text-sm font-medium">Your application is incomplete. Complete onboarding to start selling.</p>
              <Link href="/seller/onboarding" className="text-sm font-bold text-primary-ink hover:text-foreground transition">Continue →</Link>
            </div>
          )}
          {seller.status === 'changes_required' && (
            <div className="mb-6 bg-white border border-orange-200 rounded-2xl p-5">
              <p className="text-foreground font-bold text-sm mb-1">Action Required</p>
              <p className="text-muted-foreground text-sm">{seller.changes_requested}</p>
              <Link href="/seller/onboarding" className="mt-2 inline-block text-sm font-bold text-primary-ink hover:text-foreground transition">Update application →</Link>
            </div>
          )}
          {seller.status === 'rejected' && (
            <div className="mb-6 bg-white border border-red-200 rounded-2xl p-5">
              <p className="text-foreground font-bold text-sm mb-1">Application Rejected</p>
              <p className="text-muted-foreground text-sm">{seller.rejection_reason}</p>
            </div>
          )}
          ...
          {!isApproved && (seller.status === 'submitted' || seller.status === 'under_review') && (
            <div className="bg-white rounded-2xl border border-border p-8 text-center">
              ...
              <h3 className="text-lg font-black text-foreground mb-1">Application Under Review</h3>
              <p className="text-muted-foreground text-sm">We'll notify you once your application is reviewed. This typically takes 2-3 business days.</p>
            </div>
          )}
        </div>
      </div>
...
function ActionCard({ title, desc, href, icon }: { title: string; desc: string; href: string; icon: string }) {
  return (
    <Link href={href} className="bg-white rounded-2xl border border-border p-5 hover:border-primary-outline transition group">
      <div className="text-2xl mb-2">{icon}</div>
      <h3 className="font-bold text-foreground group-hover:text-primary-ink transition">{title}</h3>
