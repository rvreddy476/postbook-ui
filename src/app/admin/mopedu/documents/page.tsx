...
import {
  useMopeduDocumentsQueue,
  useRejectDocument,
  useVerifyDocument,
} from "@/hooks/useMopeduAdmin"
import type { DocumentQueueRow } from "@/lib/mopedu_api"
...
function MopeduDocumentsQueueContent() {
  ...
  const list = useMopeduDocumentsQueue(params)
  const verifyM = useVerifyDocument()
  const rejectM = useRejectDocument()

  const [selected, setSelected] = useState<DocumentQueueRow | null>(null)
  ...
  function setStatus(next: string) {
    ...
    p.set("status", next)
    router.push(`/admin/mopedu/documents?${p.toString()}`)
    setSelected(null)
  }
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-divider bg-brand-card shadow-sm">
          {list.isLoading ? (
            ...
            <EmptyState
              ...
              body="No documents matching this filter."
            />
          ) : (
                ...
                {items.map((d) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      ...
                      {d.partner_id ? (
                        <Link
                          href={`/admin/mopedu/partners/${d.partner_id}`}
                          onClick={(e) => e.stopPropagation()}
                          className="text-[11px] text-brand-text/55 underline-offset-2 hover:underline"
                        >
                          View partner
                        </Link>
                      ) : null}
          ...
          {selected ? (
            <div className="rounded-2xl border border-brand-divider bg-brand-card p-4 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-brand-text">
                    {selected.partner_name ?? "Unknown partner"}
                  </p>
                </div>
                <button
                  type="button"
              ...
              <dl className="mt-3 space-y-1.5 text-xs">
                ...
                <div className="flex justify-between gap-2">
                  <dt className="text-brand-text/60">Number</dt>
                  <dd className="font-mono">
                    {maskDocNumber(selected.document_number)}
                  </dd>
                </div>
                {selected.expires_at ? (
                  <div className="flex justify-between gap-2">
                    <dt className="text-brand-text/60">Expires</dt>
                    <dd>{formatDate(selected.expires_at)}</dd>
                  </div>
                ) : null}
                <div className="flex justify-between gap-2">
                  <dt className="text-brand-text/60">Uploaded</dt>
