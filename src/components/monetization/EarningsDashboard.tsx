        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Link
                    ...
                    className="flex items-center gap-3 px-5 py-4 rounded-2xl bg-brand-card border border-border shadow-xs hover:shadow-md hover:border-primary-outline transition-all duration-300 group"
                >
                    <div className="w-10 h-10 rounded-xl bg-linear-to-br from-primary to-primary-ink flex items-center justify-center shadow-lg shadow-[#D4A574]/20 group-hover:scale-105 transition-transform duration-200">
                ...
                <Link
                    ...
                    className="flex items-center gap-3 px-5 py-4 rounded-2xl bg-brand-card border border-border shadow-xs hover:shadow-md hover:border-primary-outline transition-all duration-300 group"
                >
                    <div className="w-10 h-10 rounded-xl bg-linear-to-br from-primary-ink to-primary-hover flex items-center justify-center shadow-lg shadow-[#7B5B3A]/20 group-hover:scale-105 transition-transform duration-200">
                        <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
            ...
            <div className="bg-brand-card rounded-2xl border border-border p-6 shadow-xs">
                <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 rounded-lg bg-background border border-border flex items-center justify-center">
                        <svg className="w-4 h-4 text-primary-ink" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                ...
                <div className="flex items-end gap-2 h-40 pt-4 px-2">
                    {[35, 55, 45, 70, 60, 80, 65, 90, 75, 85, 70, 95].map((h, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-1">
                            <div
                                className="w-full rounded-t-lg bg-linear-to-t from-primary to-primary/40 transition-all duration-500"
                                style={{ height: `${h}%` }}
                            />
                            <span className="text-[7px] font-bold text-primary-ink">
                ...
                <div className="bg-brand-card rounded-2xl border border-border p-6 shadow-xs">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-black text-foreground">Your Tiers</h3>
                        </div>
                        <Link
                            href="/monetization/tiers"
                    ...
                    <div className="space-y-2">
                        {dashboard.tiers.map((tier) => (
                            <div key={tier.id} className="flex items-center justify-between px-4 py-3 rounded-xl bg-background border border-border">
                                <div className="flex items-center gap-3">
                                    <span className={`w-2 h-2 rounded-full ${tier.is_active ? "bg-emerald-400" : "bg-slate-300"}`} />
                                    <span className="text-xs font-black text-foreground">{tier.name}</span>
                                </div>
                                <div className="flex items-center gap-4">
                                    <span className="text-[9px] font-bold text-muted-foreground">{tier.subscriber_count} subs</span>
