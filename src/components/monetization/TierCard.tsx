...
const TierCard: React.FC<TierCardProps> = ({ tier, onEdit, onToggleActive, isUpdating }) => {
    ...
    return (
        <div className={`bg-brand-card rounded-2xl border overflow-hidden shadow-xs hover:shadow-md transition-all duration-300 ${
            tier.is_active ? "border-border" : "border-brand-divider opacity-70"
        }`}>
            {/* Top accent */}
            <div className={`h-1 ${tier.is_active ? "bg-linear-to-r from-primary to-primary-ink" : "bg-slate-300"}`} />
            ...
            <div className="p-5">
                ...
                <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-lg font-black text-primary-ink">
                            {sym}{tier.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            <span className="text-[9px] font-bold text-muted-foreground ml-1">/ month</span>
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 ml-3">
                        {onEdit && (
                            <button
                                onClick={() => onEdit(tier)}
                                className="p-2 rounded-xl text-muted-foreground hover:bg-background hover:text-foreground transition-colors duration-200"
                                title="Edit tier"
                            >
                        ...
                        {onToggleActive && (
                            ...
                            >
                                {isUpdating ? (
                                    <div className="w-4 h-4 border-2 border-primary-outline border-t-transparent rounded-full animate-spin" />
                                ) : tier.is_active ? (
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                ...
                <div className="flex items-center gap-2 mb-4">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-background border border-border">
                        <svg className="w-3.5 h-3.5 text-primary-ink" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                            ...
                            <ul className="space-y-1.5 ml-1">
                                {tier.perks.map((perk, i) => (
                                    <li key={i} className="flex items-start gap-2">
                                        <svg className="w-3.5 h-3.5 text-primary-ink mt-0.5 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                        </svg>
                                        <span className="text-xs font-bold text-foreground">{perk}</span>
                                    </li>
                                ))}
                            </ul>
                        )}
