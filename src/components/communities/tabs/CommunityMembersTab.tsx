        ...
        {filters.filter(f => f.key !== 'pending' || canAdmin).map(f => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${
              filter === f.key ? 'bg-brand-text text-brand-bg' : 'bg-white border border-brand-divider text-brand-text/60 hover:bg-brand-bg'
            }`}
          >
            {f.label}
          </button>
...
function MemberRow({ member, communityId, viewerRole, canAdmin }: {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-white border border-brand-divider">
