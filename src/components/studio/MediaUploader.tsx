    ...
    return (
      <div className="animate-fadeIn">
        <div
          ...
          style={{
            borderColor: isDragging ? accentColor : '#F0E6DC',
            background: isDragging ? `${accentColor}06` : '#FAFAFA',
          }}
        >
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-[13px] font-medium" style={{ color: '#3C2415' }}>
                {isDragging ? 'Drop to upload' : isVideo ? 'Add your video' : 'Add photos'}
              </p>
              <p className="text-[11px] mt-1" style={{ color: '#C4B5A6' }}>Drag here or click to browse</p>
            </div>
            <div className="flex gap-1.5 flex-wrap justify-center">
              {[formats, countLabel, limits].map((label, i) => (
                <span
                  ...
                  style={{ background: '#FAF5F0', color: accentColor, borderColor: '#F0E6DC' }}
                >
                  {label}
                </span>
              ))}
    ...
    <div className="space-y-2 animate-fadeIn">
      <div className={`grid gap-2 ${files.length === 1 ? '' : 'grid-cols-2'}`}>
        ...
        {!isVideo && files.length < maxFiles && (
          <div
            onClick={() => fileInputRef.current?.click()}
            className="rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-[#FAF5F0] transition-all gap-1"
            style={{ aspectRatio: '4/3', borderColor: '#F0E6DC' }}
          >
            <Plus className="w-5 h-5" style={{ color: '#C4B5A6' }} />
            <span className="text-[9px]" style={{ color: '#C4B5A6' }}>Add more</span>
          </div>
        )}
      </div>
      <input
