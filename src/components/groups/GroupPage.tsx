...
export default function GroupPage({ groupId, handle, onBack }: GroupPageProps) {
  ...
  if (!group) {
    return (
      <div className="max-w-5xl mx-auto py-24 text-center">
        ...
        <p className="text-slate-400 mt-2 text-sm max-w-sm mx-auto">This group may have been deleted or you don&apos;t have access.</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-100 mt-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <aside className="hidden lg:block space-y-4">
              ...
              <div className="bg-white rounded-xl border border-slate-100 p-4">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">About this group</h3>
                {group.description && (
                  <p className="text-sm text-slate-500 leading-relaxed line-clamp-4 mb-3">{group.description}</p>
                )}
                <div className="space-y-2 text-xs text-slate-400">
                  <div className="flex items-center gap-2">
                    <Users className="w-3.5 h-3.5" />
                    <span><b className="text-slate-600">{group.member_count}</b> members</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MessageSquare className="w-3.5 h-3.5" />
                    <span><b className="text-slate-600">{group.post_count}</b> posts</span>
                  </div>
                </div>
              </div>
              ...
              {adminsAndMods.length > 0 && (
                <div className="bg-white rounded-xl border border-slate-100 p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Admins & Moderators</h3>
              ...
              {rules && rules.length > 0 && (
                <div className="bg-white rounded-xl border border-slate-100 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Group Rules</h3>
                  ...
                  <div className="space-y-2">
                    {rules.slice(0, 3).map((rule, i) => (
                      <div key={rule.id} className="flex items-start gap-2">
                        <span className="flex-shrink-0 w-5 h-5 rounded-full bg-slate-50 flex items-center justify-center text-[10px] font-bold text-slate-400 mt-0.5">
                          {i + 1}
                        </span>
                        <p className="text-xs text-slate-500 leading-relaxed">{rule.title}</p>
                      </div>
                    ))}
                    {rules.length > 3 && (
