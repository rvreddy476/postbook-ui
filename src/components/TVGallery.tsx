  ...
  return (
    <div className="animate-fadeIn">
      <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary-ink text-white rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg">
        ...
        <h2 className="text-2xl sm:text-3xl font-black text-brand-text tracking-tighter italic">VChat TV</h2>
      </div>

      {VIDEOS.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-brand-text/60">
          <svg className="w-16 h-16 mb-4 text-brand-text/30" fill="currentColor" viewBox="0 0 24 24"><path d="M10 8l6 4-6 4V8z" /><path d="M2 7v10a2 2 0 002 2h16a2 2 0 002-2V7a2 2 0 00-2-2H4a2 2 0 00-2 2z" /></svg>
          ...
          {VIDEOS.map((video) => (
            <div key={video.id} className="group cursor-pointer">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-brand-text leading-tight mb-1 group-hover:text-brand-text transition-colors truncate max-w-[240px]">
                    {video.title}
                  </h3>
                  <p className="text-[11px] font-bold text-brand-highlight tracking-widest">{video.author}</p>
