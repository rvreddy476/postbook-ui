            ...
            <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
                    activeReaction
                        ...
                        : 'text-brand-text/60 hover:bg-brand-divider'
            ...
            <AnimatePresence>
                {isOpen && !disabled && (
                    <motion.div
                        ...
                        className="absolute bottom-full left-0 mb-2 bg-brand-card rounded-full shadow-lg border border-brand-divider px-2 py-1.5 flex gap-0.5 z-50"
                    >
                        {REACTIONS.map((reaction) => (
                            <motion.button
                                ...
                                className={`w-9 h-9 flex items-center justify-center rounded-full text-xl transition-colors ${
                                    currentReaction === reaction.type ? 'bg-blue-50' : 'hover:bg-brand-secondary'
                                }`}
