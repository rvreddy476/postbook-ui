  ...
  return (
    <Link
      ...
      className={`group relative flex h-11 w-11 items-center justify-center rounded-2xl transition-all duration-200 ${
        active
          ? "bg-linear-to-br from-brand-text to-black text-white shadow-[0_4px_12px_-2px_rgba(0,0,0,0.4)]"
          : "text-muted-foreground dark:text-muted-foreground hover:bg-brand-secondary dark:hover:bg-[#2A2740] hover:text-brand-text"
      }`}
      aria-label={label}
    >
      {icon}
...
function PostTubeHeader({ dark, onToggleDark }: { dark: boolean; onToggleDark: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-50 h-[68px] border-b border-border dark:border-[#2A2740] bg-brand-card/80 dark:bg-[#13111C]/80 backdrop-blur-xl">
      <div className="mx-auto flex h-full max-w-[1440px] items-center px-6">
        ...
        <div className="flex w-[200px] shrink-0 items-center">
          ...
          >
            <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-linear-to-br from-brand-text to-black shadow-[0_2px_8px_-1px_rgba(0,0,0,0.35)]">
              <Tv2 className="h-[18px] w-[18px] text-white" />
              <div className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-warning ring-2 ring-white dark:ring-[#13111C]" />
            </div>
        ...
        <div className="flex flex-1 justify-center">
          <form onSubmit={onSubmit} className="w-[min(540px,48vw)]">
            <div className={`relative rounded-2xl transition-all duration-300 ${searchFocused ? "shadow-[0_0_0_3px_rgba(0,0,0,0.12)]" : ""}`}>
              <Search className="pointer-events-none absolute left-4 top-1/2 h-[15px] w-[15px] -translate-y-1/2 text-[#B0ADBE] dark:text-muted-foreground" />
              <input
                value={search}
                ...
                className="h-11 w-full rounded-2xl border border-border dark:border-[#2A2740] bg-[#F8F7FC] dark:bg-[#1C1A28] pl-11 pr-4 text-[13px] text-foreground dark:text-[#EEEDF5] placeholder:text-[#B0ADBE] dark:placeholder:text-muted-foreground outline-hidden transition-all duration-300 focus:border-brand-text/30 focus:bg-brand-card dark:focus:bg-[#221F32]"
              />
            </div>
          </form>
        </div>
        ...
        <div className="flex w-[240px] shrink-0 items-center justify-end gap-1.5">
          ...
          <button
            type="button"
            className="relative flex h-10 w-10 items-center justify-center rounded-2xl text-muted-foreground dark:text-muted-foreground transition-all hover:bg-brand-secondary dark:hover:bg-[#2A2740] hover:text-brand-text"
            aria-label="Notifications"
          >
            <Bell className="h-[18px] w-[18px]" />
            <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-warning ring-2 ring-white dark:ring-[#13111C]" />
          </button>
...
function PostTubeSideNav() {
  ...
  return (
    <aside className="flex h-full w-[68px] shrink-0 flex-col items-center border-r border-border dark:border-[#2A2740] bg-[#FCFBFF] dark:bg-[#16141F] py-4 gap-1.5">
      <NavIcon
        href="/posttube"
      ...
      <div className="my-1.5 h-px w-7 rounded-full bg-secondary dark:bg-[#2A2740]" />
