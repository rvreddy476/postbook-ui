      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className="absolute right-0 mt-3 w-64 rounded-[1.5rem] border border-slate-200/50 bg-white/95 p-1.5 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.12)] backdrop-blur-3xl z-[1000]"
          >
            {/* Identity */}
            <div className="border-b border-slate-100/60 p-3 mb-1">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <h4 className="truncate text-[11px] font-black uppercase tracking-widest text-slate-900">
                    {displayName}
                  </h4>
                  {handle ? (
                    <p className="truncate text-[10px] text-slate-400">@{handle}</p>
                  ) : null}
            ...
            <div className="space-y-0.5">
              {menuItems.map((item) => (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)}>
                  <div className="group flex w-full cursor-pointer items-center gap-3 rounded-xl px-3 py-2 transition-all hover:bg-slate-50">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-lg bg-slate-50 text-slate-400 transition-colors ${item.hoverColor}`}>
                      <item.icon className="h-4 w-4" />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 group-hover:text-slate-950">
                      {item.label}
                    </span>
                  </div>
                </Link>
            ...
            {/* Logout */}
            <div className="mt-1 border-t border-slate-100/60 pt-1">
              <button
                type="button"
                onClick={() => {
                  setOpen(false);
