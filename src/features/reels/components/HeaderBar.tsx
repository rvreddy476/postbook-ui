...
export function HeaderBar({
  ...
  return (
    <header className="sticky top-0 z-50 h-[72px] bg-white border-b border-[#EDEDEF]">
      <div className="mx-auto flex h-full max-w-[1400px] items-center px-6">
        ...
        <div className="flex flex-1 justify-center">
          <form onSubmit={onSearchSubmit} className="w-[min(560px,50vw)] max-w-full">
            <div className="relative">
              <Search className="pointer-events-none absolute left-4 top-1/2 h-[15px] w-[15px] -translate-y-1/2 text-slate-400" />
              <input
                value={searchValue}
                onChange={(event) => onSearchValueChange(event.target.value)}
                placeholder="Search reels, creators, hashtags..."
                className="h-[42px] w-full rounded-full border border-transparent bg-[#F5F5F7] pl-11 pr-4 text-[13px] text-slate-800 placeholder:text-slate-400 outline-none transition-all duration-200 focus:border-slate-200 focus:bg-white focus:shadow-[0_0_0_4px_rgba(0,0,0,0.03)]"
              />
            </div>
          </form>
        </div>
        ...
        <div className="flex w-[240px] shrink-0 items-center justify-end gap-2">
          ...
          <button
            type="button"
            className="relative flex h-10 w-10 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-50 hover:text-slate-600"
